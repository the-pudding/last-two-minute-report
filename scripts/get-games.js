...
function savePDFs(urls, cb) {
	let i = 0;
	const next = () => {
		const downloadUrl = urls[i].includes('ak-static') ? urls[i].replace('http', 'http') : urls[i];
...
function convertToText() {
	const command =
		'cd processing/pdf; for file in *.pdf; do pdftotext -layout -nopgbrk "$file" "../text/$file.txt"; done';
	shell.exec(command, { silent: true });
}

function convertToHTML() {
	const command =
		'cd processing/pdf; for file in *.pdf; do pdftohtml -s -i -nomerge -noframes "$file" "../html/$file.html"; done';
	shell.exec(command, { silent: true });
}
...
function init() {
	const command =
		'curl -o processing/archive.html http://official.nba.com/2017-18-nba-officiating-last-two-minute-reports/';
